...
export const createCompanion = async (
  ...
  if (formData.lessonPlanFile?.length) {
    const file = formData.lessonPlanFile[0];
    ...
    const { publicUrl } = supabase.storage.from("lesson-plans").getPublicUrl(path).data;

    lesson_plan_url = publicUrl;
  }

  const { lessonPlanFile, ...rest } = formData;

  const { data, error } = await supabase
    .from('companions')
    .insert({ ...rest, author, lesson_plan_url })
    .select();
